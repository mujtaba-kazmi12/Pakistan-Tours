...
const TourDestination = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    let pictureUrl = '';
    if (picture) {
      const pictureRef = ref(storage, `tourDestinations/${picture.name}`);
    ...
    let picture2Url = '';
    if (picture2) {
      const picture2Ref = ref(storage, `tourDestinations/${picture2.name}`);
      const picture2Snapshot = await uploadBytes(picture2Ref, picture2);
      picture2Url = await getDownloadURL(picture2Snapshot.ref);
    }

    try {
      const docRef = await addDoc(collection(db, "TourDestination"), {
        ...formData,
        pictureUrl,
        picture2Url,
        feedback: [],
  ...
  useEffect(() => {
    const fetchDestinations = async () => {
      const querySnapshot = await getDocs(collection(db, "TourDestination"));
  ...
  const handleConfirmDelete = async () => {
    ...
    selectedForDeletion.forEach((id) => {
      const docRef = doc(db, "TourDestination", id);
      batch.delete(docRef);
    });
